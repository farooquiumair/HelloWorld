...
//import { Scenario, type Options } from 'k6/options'
...
// =================== START: NEW DYNAMIC PROFILE LOGIC ===================
// This block intelligently decides which profile system to use.

// Define the valid dynamic test types for type safety.
type DynamicTestType = 'stress' | 'peak' | 'soak' | 'spike';

// Type guard to check if a string is a valid dynamic test type.
function isDynamicTestType(value: string): value is DynamicTestType {
  return ['stress', 'peak', 'soak', 'spike'].includes(value);
}

// Declare variables with their explicit types.
let loadProfile: Profile;             // <--- FIX 1: Explicitly type loadProfile
let scenarios: Record<string, Scenario>; // <--- FIX 2: Explicitly type scenarios

const testType = (__ENV.TEST_TYPE || '').toLowerCase();

// --- The Main Decision ---
// If TEST_TYPE is a dynamic one, use the new system.
if (isDynamicTestType(testType)) {
    ...
    // Create a mock 'loadProfile' object so the 'describeProfile' function doesn't fail.
    loadProfile = {
        name: `Dynamic - ${testType}`,
        scenarios: scenarios
    };
} else {
    // Otherwise, use the ORIGINAL, UNCHANGED logic.
    console.log(`[Profile] TEST_TYPE not set or invalid. Using STATIC profile selector.`);
    loadProfile = selectProfile(profiles);
    scenarios = loadProfile.scenarios;
}
// ==================== END: NEW DYNAMIC PROFILE LOGIC ====================
